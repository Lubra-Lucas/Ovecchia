...
def install_telegram_dependencies():
    """Instala dependências do Telegram se necessário"""
    try:
        import telegram
        st.success("✅ python-telegram-bot já instalado")
        return True
    except ImportError:
        st.info("📦 Instalando python-telegram-bot...")
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "python-telegram-bot"])
            st.success("✅ python-telegram-bot instalado com sucesso!")
            return True
        except Exception as e:
            st.error(f"❌ Erro ao instalar python-telegram-bot: {e}")
            return False

def create_telegram_bot_file():
    """Cria o arquivo do bot do Telegram"""
...
'''
    ...
    st.success("📄 Arquivo do bot criado: telegram_bot.py")
...
with tab5:
    ...
    with col1:
        ...
        bot_status = "🟢 Online" if check_bot_status() else "🔴 Offline"
        st.markdown(f"""
        <div class="metric-card">
            <h4 style="margin: 0; color: #1f77b4;">Status</h4>
            <h2 style="margin: 0; color: #333;">{bot_status}</h2>
        </div>
        """, unsafe_allow_html=True)
